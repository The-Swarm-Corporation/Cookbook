...
def create_tree(path: Path, tree: Tree) -> None:
    ...
    try:
        # Sort directories first, then files, but only include .py and .ipynb files
        paths = sorted(
            path.iterdir(),
        ...
        for item in paths:
            ...
            # Only process directories and .py/.ipynb files
            if item.is_dir():
                # Check if directory contains any .py or .ipynb files
                has_py_files = any(
